...
library(PRROC)     # 用于PR曲线
library(readxl)    # 用于读取Excel文件
library(ROSE)      # 用于过采样
library(vroom)     # 用于快速读取数据
library(caret)     # 用于数据处理

# 读取三个胁迫条件下的调控关系数据（第三列为权重/得分）
Alkalinity_data <- vroom("Alkalinity_significant_edges.csv", show_col_types = FALSE)[,1:3] %>%
  rename(TF = 1, Target = 2, EdgeWeight = 3)

Aridity_data <- vroom("Aridity_significant_edges.csv", show_col_types = FALSE)[,1:3] %>%
  rename(TF = 1, Target = 2, EdgeWeight = 3)

Cold_data <- vroom("Cold_significant_edges.csv", show_col_types = FALSE)[,1:3] %>%
  rename(TF = 1, Target = 2, EdgeWeight = 3)
