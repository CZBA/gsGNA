# ============================================================================
# GRN Topological Characterisation & Random Network Permutation Analysis
# ============================================================================
...
# ====================== Utility Functions: Edge Reading, Graph Construction, Topology Calculation ======================
#' Read edge list from CSV file
#' @param file_path Path to CSV edge table
#' @param weight_threshold Optional cutoff for edge weight filtering
...
#' Build directed gene regulatory network graph
#' @param edge_list TF-Target edge table with weight column
...
#' Calculate topological metrics for a single GRN, return raw degree vector for plotting
...
#' Generate random GNM network preserving original positive/negative weight distribution
#' @param original_graph Input GRN igraph object
...
#' Multiple permutation of random networks, output mean and SD of topological metrics
#' @param original_graph igraph GRN object
#' @param n_perm Number of random permutations
...
#' Compare real GRN against permuted random networks for one stress condition
#' @param original_graph igraph GRN object
#' @param name Label for stress condition
#' @param n_perm Number of random permutations
compare_networks <- function(original_graph, name, n_perm = 20) {
  cat(paste0("===== Analyzing ", name, " network, permutation runs: ", n_perm, " =====\n"))
...
# ====================== Visualization 1: Topological Metrics Bar Plot with Error Bars ======================
#' Plot bar chart comparing topology metrics between real and permuted random networks across stresses
#' @param comparison_list List output from compare_networks()
#' @param save_name Filename prefix for output PDF
...
# ====================== Visualization 2: Log-log Degree Distribution with Power-law Fitting ======================
#' Plot degree distribution on log-log scale and fit power-law for each GRN
#' @param comparison_list List output from compare_networks()
#' @param save_name Filename prefix for output PDF
...
perm_times <- 1000 # Number of random network permutations

# Read edge tables for three stress conditions, modify file paths locally
...
# Construct igraph graph objects
...
# Combine graphs and edge lists for batch processing
...
# Compare real GRNs with permuted random networks
...
cat("\n===== Start plotting topological metric bar chart =====\n")
p1 <- plot_all_topology_bar(comp_all)
print(p1)

cat("\n===== Start plotting log-log degree distribution =====\n")
p2 <- plot_degree_ggplot(comp_all)
print(p2)

cat("\nAll figures generated. PDF files saved to working directory!\n")
